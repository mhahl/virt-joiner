...
def test_ipa_resolve_srv_success(mocker):
    ...
    # Prio 10: host1, host2
    # Prio 20: host3
    record1 = MagicMock(priority=10, weight=100, port=88, target=MagicMock(to_text=lambda: "host1."))
    record2 = MagicMock(priority=10, weight=100, port=88, target=MagicMock(to_text=lambda: "host2."))
    record3 = MagicMock(priority=20, weight=100, port=88, target=MagicMock(to_text=lambda: "host3."))
...
def test_get_ipa_client_retry_flow(mocker):
    ...
    mocker.patch.dict("app.services.ipa.CONFIG", {
        "DOMAIN": "example.com",
